...
import FavoriteCities from "./Feature2";
import Feutre4 from "./Feutre4"; // Import the new Feutre4 component
import NightModeToggle from "./Feature5";
...
function Grp204WeatherApp() {
  ...
  const [isNightMode, setIsNightMode] = useState(false); // Added state for night mode
  ...
  const detectLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          alert(
            "Impossible de détecter votre localisation. Veuillez entrer une ville manuellement."
          );
        }
      );
    } else {
  ...
  return (
    <div className={`App ${isNightMode ? "night-mode" : ""}`}>
      ...
      {/* 5-Day Forecast */}
      {weather.forecast.length > 0 && <Forecast forecast={weather.forecast} />}
    </div>
  );
}
